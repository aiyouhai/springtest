...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function(exports) {
    ...
    var Tab = function() {
        this.config = {
            elem: undefined,
            mainUrl: 'app/templates/manage/home/home.html',
            renderType: 'iframe',
            openWait: true
        };
        /*this.v = '0.0.5';*/
    ...
    var _tab = {
        ...
        tabAdd: function(options) {        	
            ...
            if (_config.openWait)
                loadIndex = that.load();
            var titleHtm = ['<li class="layui-this" lay-id="' + id + '" data-url="/' + urlHome + '' + url + '">'];
            if (uid) {
                if (uid.indexOf('fa-') !== -1) {
                    titleHtm.push('<i class="iconfont ' + uid + '" aria-hidden="true"></i>');
                } else {
                    titleHtm.push('<i class="iconfont">' + uid + '</i>');
                }
            }
            titleHtm.push('&nbsp;' + text);
            ...
            switch (_config.renderType) {
                case renderType.page:
                    ...
                    break;
                case renderType.iframe:
                    contentHtm = contentHtm.replace('{{content}}', '<iframe src="/' + urlHome + '' + url + '"></iframe>');
                    break;
            }
            //追加htm
